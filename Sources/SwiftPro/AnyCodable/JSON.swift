...
//
//
//  Created by Dmitry Mikhaylov on 11.03.2024.
//
...
public struct JSON: @unchecked Sendable, Codable {
    /**
    ...
    public fileprivate(set) var _type: Type = .null
...
extension JSON {
    ...
    fileprivate subscript(key key: String) -> JSON {
        ...
        set {
            if _type == .dictionary && newValue.error == nil {
...
extension JSON: Swift.RawRepresentable {
    public init?(rawValue: Any) {
        if JSON(rawValue)._type == .unknown {
            return nil
        } else {
            self.init(rawValue)
        }
...
// MARK: - Printable, Log.global.debugable
...
extension JSON {
    ...
    public var array: [JSON]? {
        return _type == .array ? rawArray.map { JSON($0) } : nil
    }
