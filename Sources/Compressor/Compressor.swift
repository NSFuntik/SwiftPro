...
extension Data {
    ...
    fileprivate func compress(_ compression: CompressionAlgorithm, operation: Operation, bufferSize: size_t) throws -> Data? {
        ...
        defer {
            dstBufferPointer.deallocate() /* .deallocate(capacity: bufferSize) */
        }
        ...
        /// ``Stream setup after compression_stream_ini``
        /// - `Explanation`:
        ///   - -  `withUnsafeBytes` now takes a closure with a single parameter of type UnsafeRawBufferPointer.
        ///   - -  `baseAddress` provides a pointer to the memory buffer’s first byte, or nil if the buffer is empty.
        ///   - - `assumingMemoryBound(to:)` is used to bind the memory to the expected type (UInt8 in this case).
        /// This should be used only when you are sure of the underlying type because this method does not perform any type checks or memory rebindings.
        withUnsafeBytes { rawBufferPointer in
            // Get a pointer to the start of the data as UInt8
